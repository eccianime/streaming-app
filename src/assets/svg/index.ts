export { default as AppleLogo } from './appleLogo.svg';
export { default as GoogleLogo } from './googleLogo.svg';
export { default as FacebookLogo } from './facebookLogo.svg';
export { default as LetsInImage } from './letsIn.svg';
export { default as Logo } from './logo.svg';
export { default as UserImage } from './user.svg';

export { default as UserIcon } from './user1.svg';
export { default as UserFillIcon } from './user-fill.svg';
export { default as HomeIcon } from './home.svg';
export { default as HomeFillIcon } from './home-fill.svg';
export { default as DownloadIcon } from './download.svg';
export { default as DownloadFillIcon } from './download-fill.svg';
export { default as BookmarkIcon } from './bookmark.svg';
export { default as BookmarkFillIcon } from './bookmark-fill.svg';
export { default as CompassIcon } from './compass.svg';
export { default as CompassFillIcon } from './compass-fill.svg';

export { default as BellIcon } from './bell.svg';
export { default as SearchIcon } from './search.svg';

export { default as PlayIcon } from './play.svg';
export { default as SendIcon } from './send.svg';

export { default as EqualizerIcon } from './equalizer.svg';
export { default as NotFound } from './not-found.svg';
